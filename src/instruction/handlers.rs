use crate::{ Instruction, AddressMode, Modifier };
...
#[inline]
fn follow_address(reg: isize, mode: AddressMode, cur_address: usize, max: usize, memory: &[Instruction]) -> usize {
    use AddressMode::*;

    match mode {
        // Direct will always return an insutrction
        Direct => {
            relative_address(max, cur_address, reg)
        },
...
pub fn mov(instruction: Instruction, cur_address: usize, max: usize, memory: &mut Vec<Instruction>) {
    use Modifier as m;

    let Instruction { a_reg, a_mode, b_reg, b_mode, modifier, .. } = instruction;
...
pub fn jmp(instruction: Instruction, cur_address: usize, max: usize, memory: &mut Vec<Instruction>) -> usize {
